...
class CreatDatas(object):
    ...
    # 输出json数据
    def creatJson(self, url_path, json_name, url_name2):

        if not os.path.exists(url_path):
            os.makedirs(url_path)
        with open(os.path.join(url_path, '{}.json'.format(url_name2+json_name)), 'w', encoding='utf_8') as w:
            json.dump(self.questions_all, w, ensure_ascii=False)

    # 输出url
    def creatUrl(self, url_name1, url_name2):
        # print(self.questions_all)
